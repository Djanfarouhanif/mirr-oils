...
function renderProductBanner() {
  ...
  b.innerHTML = DB.produits.filter(p => p.name !== 'Reglement client').map(p => {
...
function refreshStats() {
  ...
  let sales=DB.sales.filter(s=>s.date>=from&&s.date<=to);
  if(pt) sales=sales.filter(s=>s.priceType===pt);
  if(pp) sales=sales.filter(s=>s.prod===pp);
  let deps=DB.depenses.filter(d=>d.date>=from&&d.date<=to);
...
function fillProductOptions() {
  ...
  if(!sel)return;
  const cur=sel.value;
  sel.innerHTML='<option value="">-- Selectionner --</option>'+DB.produits.map(p=>`<option value="${p.name}">${p.name}</option>`).join('');
  if(cur)sel.value=cur;
  const sp=document.getElementById('statProd');
  if(sp){sp.innerHTML='<option value="">Tous</option>'+DB.produits.map(p=>`<option value="${p.name}">${p.name}</option>`).join('');}
}
function fillPrice() {
  const pn=document.getElementById('vProd').value;
  const pt=document.getElementById('vPriceType').value;
  const prod=DB.produits.find(p=>p.name===pn);
  if(prod) {
...
function saveSale() {
  ...
  if(editingSaleId) {
    ...
    if(idx>=0) DB.sales[idx]={...DB.sales[idx],date,prod,qty,priceType:pt,unitPrice:unit,remise:rem,clientName:cli,amount:amt,note};
    toast('Vente modifiee','success');
  } else {
    DB.sales.unshift({id:DB.nextId++,date,prod,qty,priceType:pt,unitPrice:unit,remise:rem,clientName:cli,amount:amt,note});
...
function showReceipt(s) {
  document.getElementById('rNum').textContent='MO-'+new Date().getFullYear()+'-'+String(DB.nextId).padStart(4,'0');
  ...
  const rl  = document.getElementById('rRemiseLine');
  const rsl = document.getElementById('rSubtotalLine');
...
function deleteSale(id) {
  ...
  DB.sales=DB.sales.filter(s=>s.id!==id); renderSales(); updateSaleStats(); toast('Vente supprimee','success');
}
function renderSales() {
  const fd=document.getElementById('fDate').value;
  const ft=document.getElementById('fType').value;
  const fs=(document.getElementById('fSearch').value||'').toLowerCase();
  const dates=[...new Set(DB.sales.map(s=>s.date))].sort().reverse();
  ...
  if(cv)fds.value=cv;
  let rows=DB.sales.filter(s=>(!fd||s.date===fd)&&(!ft||s.priceType===ft)&&(!fs||s.prod.toLowerCase().includes(fs)||s.clientName.toLowerCase().includes(fs)));
...
function updateSaleStats() {
  const ca=DB.sales.reduce((a,s)=>a+s.amount,0), cnt=DB.sales.length;
...
function saveDepense() {
  ...
  if(editingDepId) {
    const idx=DB.depenses.findIndex(d=>d.id===editingDepId);
    if(idx>=0) DB.depenses[idx]={...DB.depenses[idx],date,cat,desig,amount:amt,note};
    toast('Depense modifiee','success');
  } else {
    DB.depenses.unshift({id:DB.nextId++,date,cat,desig,amount:amt,note});
...
function editDepense(id) {
  const d=DB.depenses.find(x=>x.id===id); if(!d)return;
...
function deleteDepense(id) {
  ...
  DB.depenses=DB.depenses.filter(d=>d.id!==id); renderDepenses(); updateDepStats(); toast('Depense supprimee','success');
}
function renderDepenses() {
  const fd=document.getElementById('fDepDate').value, fc=document.getElementById('fDepCat').value;
  const dates=[...new Set(DB.depenses.map(d=>d.date))].sort().reverse();
  const fds=document.getElementById('fDepDate'), cv=fds.value;
  fds.innerHTML='<option value="">Toutes les dates</option>'+dates.map(d=>`<option value="${d}">${fmtD(d)}</option>`).join('');
  if(cv)fds.value=cv;
  let rows=DB.depenses.filter(d=>(!fd||d.date===fd)&&(!fc||d.cat===fc));
...
function updateDepStats() {
  const total=DB.depenses.reduce((a,d)=>a+d.amount,0), cnt=DB.depenses.length;
...
function fillClientDropdown() {
  ...
  if(!sel)return;
  sel.innerHTML='<option value="">-- Aucun --</option>'+DB.clients.map(c=>`<option value="${c.name}">${c.name} (${ptL(c.cat)})</option>`).join('');
}
function saveClient() {
  const name=document.getElementById('cName').value;
  if(!name){toast('Nom obligatoire','error');return;}
  DB.clients.push({id:DB.nextId++,name,cat:document.getElementById('cCat').value,phone:document.getElementById('cPhone').value,email:document.getElementById('cEmail').value,addr:document.getElementById('cAddr').value,priceType:document.getElementById('cPriceType').value,solde:parseFloat(document.getElementById('cSolde').value)||0,note:document.getElementById('cNote').value});
  closeModal('modalClient'); renderClients(); updateClientCounts(); fillClientDropdown(); toast('Client ajoute !','success');
}
function deleteClient(id) {
  if(!confirm('Supprimer ?'))return;
  DB.clients=DB.clients.filter(c=>c.id!==id); renderClients(); updateClientCounts(); fillClientDropdown(); toast('Client supprime','success');
}
function filterClients(cat,el) {
  document.querySelectorAll('#page-clients .tab-item').forEach(t=>t.classList.remove('active'));
  el.classList.add('active'); DB.clientFilter=cat; renderClients();
}
function renderClients() {
  const fs=(document.getElementById('fClientSearch')?.value||'').toLowerCase();
  let rows=DB.clients.filter(c=>(!DB.clientFilter||c.cat===DB.clientFilter)&&(!fs||c.name.toLowerCase().includes(fs)||(c.phone||'').includes(fs)));
...
function saveProduit() {
  ...
  DB.produits.push({id:DB.nextId++,name,cond:document.getElementById('pCond').value,pc,pm:parseFloat(document.getElementById('pPM').value)||0,pg:parseFloat(document.getElementById('pPG').value)||0,unit:document.getElementById('pUnit').value,stock:parseInt(document.getElementById('pStock').value)||0,alerte:parseInt(document.getElementById('pAlerte').value)||0});
  closeModal('modalProduit'); renderProduits(); fillProductOptions(); toast('Produit ajoute !','success');
}
function deleteProduit(id) {
  if(!confirm('Supprimer ?'))return;
  DB.produits=DB.produits.filter(p=>p.id!==id); renderProduits(); fillProductOptions(); toast('Produit supprime','success');
}
function renderProduits() {
  document.getElementById('prodBody').innerHTML=DB.produits.map(p=>{
...
function loadWeekReport() {
  ...
  const depObjW = 100000;
  const depPct = Math.round(d.dep / depObjW * 100);
  const totalCA = d.ca || 1;
  // Synthèse ventes par produit (issue des données statiques de la semaine)
  const prodSynth = d.prodSynth || [];
  const prodRows = prodSynth.map((p,i) => `
    <tr>
      <td>${i+1}</td>
      <td><strong>${p.n}</strong></td>
      <td class="text-right num">${p.qte}</td>
      <td class="text-right num">${fcfa(p.ca)}</td>
      <td class="text-right">
        <div style="display:flex;align-items:center;gap:8px">
          <div style="flex:1;height:6px;background:var(--bg4);border-radius:3px;overflow:hidden"><div style="height:100%;border-radius:3px;background:var(--primary);width:${Math.round(p.ca/totalCA*100)}%"></div></div>
          <span class="fs-11">${Math.round(p.ca/totalCA*100)}%</span>
        </div>
      </td>
    </tr>`).join('');
  ...
  <div class="card mb-16">
    <div class="card-header"><div>
      <div class="card-title"><i class="ti ti-file-analytics"></i> ${d.label}</div>
      <div class="card-subtitle">Mirr Oils — Distribution Huiles Moteur &amp; Lubrifiants</div>
    </div></div>
  ...
  <div class="g2 mb-16" style="margin-bottom:16px">
    <div class="card">
      <div class="card-header"><div class="card-title"><i class="ti ti-calendar-week"></i> Récapitulatif par jour</div></div>
      <div class="tbl-wrap"><table>
        <thead><tr><th>Date</th><th class="text-right">CA (FCFA)</th><th>% CA</th><th class="text-right">Dépenses</th><th class="text-right">Solde net</th></tr></thead>
        <tbody>${d.days.map(x=>`<tr>
          <td>${x.d}</td>
          <td class="text-right num fw-600">${fmt(x.ca)}</td>
          <td class="fs-11 text-muted">${Math.round(x.ca/totalCA*100)} %</td>
          <td class="text-right num ${x.dep>0?'text-danger':''}">${x.dep>0?fmt(x.dep):'—'}</td>
          <td class="text-right num ${x.ca-x.dep>=0?'text-success':'text-danger'}">${fmt(x.ca-x.dep)}</td>
        </tr>`).join('')}</tbody>
        <tfoot><tr><td class="fw-600">TOTAL</td><td class="text-right num fw-600">${fmt(d.ca)}</td><td>100 %</td><td class="text-right num text-danger fw-600">${fmt(d.dep)}</td><td class="text-right num text-success fw-600">${fmt(d.net)}</td></tr></tfoot>
      </table></div>
    </div>
    <div class="card">
      <div class="card-header"><div class="card-title"><i class="ti ti-wallet"></i> Dépenses de la semaine</div></div>
      <div class="tbl-wrap"><table>
        <thead><tr><th>Date</th><th>Désignation</th><th class="text-right">Montant (FCFA)</th></tr></thead>
        <tbody>${d.depenses.map(x=>`<tr><td>${x.d}</td><td>${x.des}</td><td class="text-right num">${fmt(x.amt)}</td></tr>`).join('')}</tbody>
        <tfoot>
          <tr><td colspan="2">CA total</td><td class="text-right num text-success">${fmt(d.ca)}</td></tr>
          <tr><td colspan="2" class="text-danger">Total dépenses</td><td class="text-right num text-danger">${fmt(d.dep)}</td></tr>
          <tr><td colspan="2" class="fw-600">Résultat net</td><td class="text-right num text-success fw-600">${fmt(d.net)}</td></tr>
        </tfoot>
      </table></div>
    </div>
  </div>
  ${prodRows.length ? `
  <div class="card mb-16">
    <div class="card-header"><div class="card-title"><i class="ti ti-chart-bar"></i> Synthèse des ventes par produit</div></div>
    <div class="tbl-wrap"><table>
      <thead><tr><th>#</th><th>Produit</th><th class="text-right">Qté</th><th class="text-right">CA (FCFA)</th><th>% CA semaine</th></tr></thead>
      <tbody>${prodRows}</tbody>
      <tfoot><tr><td colspan="3" class="fw-600">TOTAL</td><td class="text-right num fw-600">${fmt(d.ca)}</td><td>100 %</td></tr></tfoot>
    </table></div>
  </div>` : ''}
  <div class="card">
    <div class="card-header"><div class="card-title"><i class="ti ti-target"></i> Suivi des objectifs</div></div>
    <div class="tbl-wrap"><table>
      <thead><tr><th>Indicateur</th><th class="text-right">Objectif</th><th class="text-right">Réalisé</th><th class="text-right">Écart</th><th class="text-right">Taux</th><th>Statut</th></tr></thead>
      <tbody>
        <tr>
          <td>CA Hebdomadaire</td>
          <td class="text-right num">${fmt(d.objCA)}</td>
          <td class="text-right num">${fmt(d.ca)}</td>
          <td class="text-right num ${d.ca>=d.objCA?'text-success':'text-danger'}">${d.ca>=d.objCA?'+':''}${fmt(d.ca-d.objCA)}</td>
          <td class="text-right"><span class="badge ${pct>=100?'b-success':pct>=80?'b-warning':'b-danger'}">${pct} %</span></td>
          <td>${pct>=100?'<span class="badge b-success">Objectif atteint</span>':pct>=80?'<span class="badge b-warning"> Proche</span>':'<span class="badge b-danger">Non atteint</span>'}</td>
        </tr>
        <tr>
          <td>Résultat Net</td>
          <td class="text-right num">${fmt(Math.round(d.objCA*0.875))}</td>
          <td class="text-right num">${fmt(d.net)}</td>
          <td class="text-right num ${d.net>=d.objCA*0.875?'text-success':'text-danger'}">${d.net>=d.objCA*0.875?'+':''}${fmt(d.net-Math.round(d.objCA*0.875))}</td>
          <td class="text-right"><span class="badge ${d.net>=d.objCA*0.875?'b-success':'b-warning'}">${Math.round(d.net/(d.objCA*0.875)*100)} %</span></td>
          <td>${d.net>=d.objCA*0.875?'<span class="badge b-success">Atteint</span>':'<span class="badge b-warning">À améliorer</span>'}</td>
        </tr>
        <tr>
          <td>Maîtrise dépenses (max ${fmt(depObjW)} FCFA)</td>
          <td class="text-right num">${fmt(depObjW)}</td>
          <td class="text-right num">${fmt(d.dep)}</td>
          <td class="text-right num text-success">-${fmt(depObjW-d.dep)}</td>
          <td class="text-right"><span class="badge ${depPct<=100?'b-success':'b-danger'}">${depPct} %</span></td>
          <td>${depPct<=100?'<span class="badge b-success">Budget respecté</span>':'<span class="badge b-danger">Dépassé</span>'}</td>
        </tr>
      </tbody>
    </table></div>
  </div>`;
...
function loadMonthReport() {
  ...
  const pct = Math.round(d.ca / d.objCA * 100);
  const depObj = 250000;
  const depPct = Math.round(d.dep / depObj * 100);
  const netObj = Math.round(d.objCA * 0.98);
  const netPct = Math.round(d.net / netObj * 100);
  const totalCA = d.ca || 1;
  document.getElementById('monthReportContent').innerHTML = `
  <div class="card mb-16">
    <div class="card-header"><div>
      <div class="card-title"><i class="ti ti-file-analytics"></i> Rapport mensuel — ${d.label}</div>
      <div class="card-subtitle">Mirr Oils — Distribution Huiles Moteur &amp; Lubrifiants</div>
    </div></div>
    <div class="g4" style="margin-bottom:0">
  ...
  <div class="card mb-16">
    <div class="card-header"><div class="card-title"><i class="ti ti-calendar-week"></i> Récapitulatif par semaine</div></div>
    <div class="tbl-wrap"><table>
      <thead><tr><th>Semaine</th><th class="text-right">CA (FCFA)</th><th class="text-right">Dépenses</th><th class="text-right">Résultat net</th><th>% CA mois</th><th>Statut</th></tr></thead>
      <tbody>${d.weeks.map(w => {
        const sp = Math.round(w.ca / totalCA * 100);
        const ok = w.ca >= (d.objCA / 4);
        return `<tr>
          <td>${w.s} — ${w.p}</td>
          <td class="text-right num ${ok?'fw-600':''}">${fmt(w.ca)}</td>
          <td class="text-right num text-danger">${fmt(w.dep)}</td>
          <td class="text-right num">${fmt(w.net)}</td>
          <td><div style="display:flex;align-items:center;gap:8px"><div style="flex:1;height:6px;background:var(--bg4);border-radius:3px;overflow:hidden"><div style="height:100%;border-radius:3px;background:var(--primary);width:${sp}%"></div></div><span class="fs-11">${sp} %</span></div></td>
          <td>${ok ? '<span class="badge b-success">Objectif atteint</span>' : '<span class="badge b-warning"> Sous objectif</span>'}</td>
        </tr>`;
      }).join('')}</tbody>
      <tfoot><tr><td class="fw-600">TOTAL MOIS</td><td class="text-right num fw-600">${fmt(d.ca)}</td><td class="text-right num text-danger fw-600">${fmt(d.dep)}</td><td class="text-right num text-success fw-600">${fmt(d.net)}</td><td>100 %</td><td><span class="tag">Obj : ${fmt(d.objCA)} FCFA</span></td></tr></tfoot>
    </table></div>
  </div>
  <div class="card mb-16">
    <div class="card-header"><div class="card-title"><i class="ti ti-wallet"></i> Dépenses par catégorie</div></div>
    <div class="tbl-wrap"><table>
      <thead><tr><th>Catégorie</th><th class="text-right">Montant (FCFA)</th><th>% Dépenses</th></tr></thead>
      <tbody>${d.depByCat.map(x=>`<tr>
        <td>${x.cat}</td>
        <td class="text-right num">${fmt(x.amt)}</td>
        <td><div style="display:flex;align-items:center;gap:8px"><div style="flex:1;height:6px;background:var(--bg4);border-radius:3px;overflow:hidden"><div style="height:100%;border-radius:3px;background:var(--warning);width:${Math.round(x.amt/d.dep*100)}%"></div></div><span class="fs-11">${Math.round(x.amt/d.dep*100)} %</span></div></td>
      </tr>`).join('')}</tbody>
      <tfoot><tr><td class="fw-600">TOTAL</td><td class="text-right num text-danger fw-600">${fmt(d.dep)}</td><td></td></tr></tfoot>
    </table></div>
  </div>
  <div class="card mb-16">
    <div class="card-header"><div class="card-title"><i class="ti ti-chart-bar"></i> Synthèse des ventes par produit</div></div>
    <div class="tbl-wrap"><table>
      <thead><tr><th>#</th><th>Produit</th><th class="text-right">CA (FCFA)</th><th>% CA mensuel</th></tr></thead>
      <tbody>${d.topProds.map((p, i) => `<tr>
        <td>${['1','2','3','4️','5️'][i]}</td>
        <td><strong>${p.n}</strong></td>
        <td class="text-right num">${fmt(p.ca)}</td>
        <td><div style="display:flex;align-items:center;gap:8px"><div style="flex:1;height:6px;background:var(--bg4);border-radius:3px;overflow:hidden"><div style="height:100%;border-radius:3px;background:var(--primary);width:${Math.round(p.ca/totalCA*100)}%"></div></div><span class="fs-11">${Math.round(p.ca/totalCA*100)} %</span></div></td>
      </tr>`).join('')}</tbody>
      <tfoot><tr><td colspan="2" class="fw-600">TOTAL (top 5)</td><td class="text-right num fw-600">${fmt(d.topProds.reduce((a,p)=>a+p.ca,0))}</td><td class="fs-11 text-muted">${Math.round(d.topProds.reduce((a,p)=>a+p.ca,0)/totalCA*100)} % du CA</td></tr></tfoot>
    </table></div>
  </div>
  <div class="card">
    <div class="card-header"><div class="card-title"><i class="ti ti-target"></i> Suivi des objectifs</div></div>
    <div class="tbl-wrap"><table>
      ...
      <tbody>
        <tr>
          ...
          <td class="text-right num ${d.ca>=d.objCA?'text-success':'text-danger'}">${d.ca>=d.objCA?'+':''}${fmt(d.ca-d.objCA)}</td>
          <td class="text-right"><span class="badge ${pct>=100?'b-success':pct>=80?'b-warning':'b-danger'}">${pct} %</span></td>
          <td>${pct>=100?'<span class="badge b-success"> Objectif atteint</span>':pct>=80?'<span class="badge b-warning"> Proche de l\'objectif</span>':'<span class="badge b-danger"> Non atteint</span>'}</td>
        </tr>
        <tr>
          <td>Résultat Net (obj. ${fmt(netObj)} FCFA)</td>
          <td class="text-right num">${fmt(netObj)}</td>
          <td class="text-right num">${fmt(d.net)}</td>
          <td class="text-right num ${d.net>=netObj?'text-success':'text-danger'}">${d.net>=netObj?'+':''}${fmt(d.net-netObj)}</td>
          <td class="text-right"><span class="badge ${netPct>=100?'b-success':netPct>=80?'b-warning':'b-danger'}">${netPct} %</span></td>
          <td>${netPct>=100?'<span class="badge b-success">Atteint</span>':netPct>=80?'<span class="badge b-warning"> Proche</span>':'<span class="badge b-danger"> Non atteint</span>'}</td>
        </tr>
        <tr>
          <td>Budget dépenses (max ${fmt(depObj)} FCFA)</td>
          <td class="text-right num">${fmt(depObj)}</td>
          <td class="text-right num">${fmt(d.dep)}</td>
          <td class="text-right num ${d.dep<=depObj?'text-success':'text-danger'}">${d.dep<=depObj?'-':'+'  }${fmt(Math.abs(d.dep-depObj))}</td>
          <td class="text-right"><span class="badge ${depPct<=100?'b-success':'b-danger'}">${depPct} %</span></td>
          <td>${depPct<=100?'<span class="badge b-success"> Budget respecté</span>':'<span class="badge b-danger"> Budget dépassé</span>'}</td>
        </tr>
      </tbody>
    </table></div>
  </div>`;
...
function generatePDF(type) {
  ...
  const kpis = isH
    ? [{l:'CA Hebdomadaire',v:fmt(d.ca)+' FCFA'},{l:'Dépenses',v:fmt(d.dep)+' FCFA'},{l:'Résultat net',v:fmt(d.net)+' FCFA'},{l:'Jours actifs',v:d.jours+'/5'}]
    : [{l:'CA Mensuel',v:fmt(d.ca)+' FCFA'},{l:'Dépenses',v:fmt(d.dep)+' FCFA'},{l:'Résultat net',v:fmt(d.net)+' FCFA'},{l:'Obj. atteint',v:Math.round(d.ca/d.objCA*100)+'%'}];
  const kw = CW/4;
  kpis.forEach((k,i) => {
    const x = M + i*kw;
  ...
  function tbl(hdr, rows, cw, footer) {
    ...
    let x = M + 2; hdr.forEach((h,i) => { doc.text(h, x, y+4.5); x += cw[i]; });
    y += rh;
    rows.forEach((r, ri) => {
      ...
      x = M + 2; r.forEach((c,ci) => { const s=String(c||''); doc.text(s.length>40?s.slice(0,40)+'...':s, x, y+4.5); x += cw[ci]; });
      y += rh;
    });
    if(footer) {
      ...
      x = M + 2; footer.forEach((c,ci) => { doc.text(String(c||''), x, y+4.5); x += cw[ci]; });
      y += rh;
    }
  ...
  if(isH) {
    ...
    tbl(
      ['Date','CA (FCFA)','% CA','Dépenses (FCFA)','Solde net (FCFA)'],
      d.days.map(x => [x.d, fmt(x.ca), Math.round(x.ca/d.ca*100)+'%', x.dep>0?fmt(x.dep):'—', fmt(x.ca-x.dep)]),
      [38, 34, 16, 40, 40],
      ['TOTAL', fmt(d.ca), '100%', fmt(d.dep), fmt(d.net)]
    );
    section('2. Dépenses de la semaine');
    tbl(
      ['Date','Désignation','Montant (FCFA)'],
      d.depenses.map(x => [x.d, x.des, fmt(x.amt)]),
      [25, 115, 40],
      ['','TOTAL', fmt(d.dep)]
    );
    if(d.prodSynth && d.prodSynth.length) {
      section('3. Synthèse des ventes par produit');
      tbl(
        ['#','Produit','Qté','CA (FCFA)','% CA'],
        d.prodSynth.map((p,i) => [String(i+1), p.n, String(p.qte), fmt(p.ca), Math.round(p.ca/d.ca*100)+'%']),
        [8, 90, 14, 42, 20],
        ['','TOTAL','', fmt(d.ca), '100%']
      );
    }
    ...
    const pct = Math.round(d.ca/d.objCA*100);
    tbl(
      ['Indicateur','Objectif (FCFA)','Réalisé (FCFA)','Écart (FCFA)','Taux','Statut'],
      [
        ['CA Hebdomadaire', fmt(d.objCA), fmt(d.ca), (d.ca>=d.objCA?'+':'')+fmt(d.ca-d.objCA), pct+'%', pct>=100?' Atteint':pct>=80?'Proche':' Non atteint'],
        ['Résultat Net', fmt(Math.round(d.objCA*0.875)), fmt(d.net), '', Math.round(d.net/(d.objCA*0.875)*100)+'%', d.net>=d.objCA*0.875?' Atteint':'⚠ À améliorer'],
        ['Dépenses (max)', '100 000', fmt(d.dep), '-'+fmt(100000-d.dep), Math.round(d.dep/100000*100)+'%', d.dep<=100000?' Respecté':' Dépassé'],
      ],
      [45, 28, 28, 28, 14, 33], null
    );
  } else {
    // ---- CONTENU MENSUEL ----
    section('1. Récapitulatif par semaine');
    tbl(
      ['Semaine','CA (FCFA)','Dépenses','Résultat net','% CA','Statut'],
      d.weeks.map(w => [w.s+' — '+w.p, fmt(w.ca), fmt(w.dep), fmt(w.net), Math.round(w.ca/d.ca*100)+'%', w.ca>=(d.objCA/4)?' OK':'Sous obj.']),
      [45, 30, 26, 28, 14, 33],
      ['TOTAL', fmt(d.ca), fmt(d.dep), fmt(d.net), '100%','']
    );
    section('2. Dépenses par catégorie');
    tbl(
      ['Catégorie','Montant (FCFA)','% Dépenses'],
      d.depByCat.map(x => [x.cat, fmt(x.amt), Math.round(x.amt/d.dep*100)+'%']),
      [100, 50, 30],
      ['TOTAL', fmt(d.dep), '100%']
    );
    section('3. Synthèse des ventes par produit (Top 5)');
    tbl(
      ['Rang','Produit','CA (FCFA)','% CA mensuel'],
      d.topProds.map((p,i) => [String(i+1)+'.', p.n.length>38?p.n.slice(0,38)+'...':p.n, fmt(p.ca), Math.round(p.ca/d.ca*100)+'%']),
      [10, 105, 40, 25],
      ['','TOTAL top 5', fmt(d.topProds.reduce((a,p)=>a+p.ca,0)), Math.round(d.topProds.reduce((a,p)=>a+p.ca,0)/d.ca*100)+'%']
    );
    section('4. Suivi des objectifs');
    const pct = Math.round(d.ca/d.objCA*100);
    const netObj = Math.round(d.objCA*0.98);
    const netPct = Math.round(d.net/netObj*100);
    const depPct = Math.round(d.dep/250000*100);
    tbl(
      ['Indicateur','Objectif','Réalisé','Écart','Taux','Statut'],
      [
        ['CA Mensuel', fmt(d.objCA)+' FCFA', fmt(d.ca)+' FCFA', (d.ca>=d.objCA?'+':'')+fmt(d.ca-d.objCA), pct+'%', pct>=100?' Atteint':pct>=80?' Proche':' Non atteint'],
        ['Résultat Net', fmt(netObj)+' FCFA', fmt(d.net)+' FCFA', (d.net>=netObj?'+':'')+fmt(d.net-netObj), netPct+'%', netPct>=100?' Atteint':netPct>=80?'Proche':' Non atteint'],
        ['Dépenses (max)', '250 000 FCFA', fmt(d.dep)+' FCFA', (d.dep<=250000?'-':'+')+fmt(Math.abs(d.dep-250000)), depPct+'%', depPct<=100?' Budget OK':' Dépassé'],
      ],
      [42, 32, 28, 26, 12, 36], null
    );
  }
  // ---- PIED DE PAGE ----
  ...
  for(let i=1; i<=pages; i++) {
    ...
    doc.text('Mirr Oils — Distribution Huiles Moteur & Lubrifiants — Lomé, Togo', M, 293);
    doc.text(`Page ${i}/${pages}`, W-M, 293, {align:'right'});
  }
...
function generateReceiptPDF() {
  ...
  const num=document.getElementById('rNum').textContent;
  const prod=document.getElementById('rProd').textContent;
  const qty=document.getElementById('rQty').textContent;
  const unit=document.getElementById('rUnit').textContent;
  const type=document.getElementById('rType').textContent;
  const client=document.getElementById('rClient').textContent;
...
function switchParam(p,el) {
  ...
  el.classList.add('active');
  ['general','apparence','categories'].forEach(id=>{
    const e=document.getElementById('param-'+id);if(e)e.style.display=(id===p)?'block':'none';
  });
}
...
function addCategoryPrompt() {
  ...
  if(n&&n.trim()&&!CATEGORIES.includes(n.trim())){
    CATEGORIES.push(n.trim()); renderCatList(); populateDepCat(); toast('Categorie ajoutee !','success');
  }
}
function removeCategory(name) {
  const idx=CATEGORIES.indexOf(name);
  if(idx>=0){CATEGORIES.splice(idx,1);renderCatList();populateDepCat();toast('Supprimee','success');}
}
